...
import com.smogline.api.energy.EnergyCapabilityProvider;
import com.smogline.capability.ModCapabilities;
import com.smogline.util.EnergyFormatter;
...
public class ModBatteryItem extends Item {
    ...
    public void appendHoverText(@Nonnull ItemStack stack, @Nullable Level level, @Nonnull List<Component> tooltip, @Nonnull TooltipFlag flag) {
        ...
        if (maxReceive > 0) {
            ...
            tooltip.add(Component.translatable("tooltip.smogline.battery.transfer_rate",
                    EnergyFormatter.format(maxReceive)).withStyle(ChatFormatting.GOLD));
        }
        if (maxExtract > 0) {
            // [🔥 ИЗМЕНЕНО: Вся строка теперь ChatFormatting.GOLD 🔥]
            tooltip.add(Component.translatable("tooltip.smogline.battery.discharge_rate",
    ...
    private void addEnergyTooltip(List<Component> tooltip, long stored, long max, ChatFormatting color) {
        ...
        tooltip.add(Component.translatable("tooltip.smogline.battery.stored").withStyle(color));
