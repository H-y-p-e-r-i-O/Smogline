...
public class TurretDebugRenderer_Enhanced {
    ...
    // Idle линия теперь тоже Magenta, но полупрозрачная
    ...
    private static void renderDebugInfo(PoseStack poseStack, MultiBufferSource.BufferSource bufferSource,
        ...
        // ВАЖНО: Используем debugLineStrip(2.0) как в ядерном моде.
        // Это делает линии толстыми (2px) и всегда видимыми.
        ...
        if (scanPoints != null && !scanPoints.isEmpty()) {
            for (Pair<Vec3, Boolean> point : scanPoints) {
                ...
                // Рисуем боксы линиями (вручную, т.к. renderLineBox использует lines())
        ...
        if (targetPoint != null) {
            ...
            // Теперь Magenta (IDLE_R/G/B) с переходом в прозрачность
            lineBuilder.vertex(matrix, (float)muzzlePos.x, (float)muzzlePos.y, (float)muzzlePos.z)
        ...
        // ВАЖНО: Принудительно завершаем батч, чтобы линии отрисовались "поверх" всего и сразу
        bufferSource.endBatch(renderType);
    }

    // Вспомогательный метод для рисования кубика через debugLineStrip
