...
public class TurretBulletEntity extends AbstractArrow implements GeoEntity, IEntityAdditionalSpawnData {
    ...
    protected void onHitBlock(BlockHitResult result) {
        if (!this.level().isClientSide) {
            ...
            spawnRadioParticles(result.getLocation());
            playGroundSound();
            this.discard();
        }
    }

    private void spawnRadioParticles(Vec3 pos) {
        if (this.level().isClientSide) {
            ...
            // ✅ ИСПРАВЛЕНО: Убран флаг boolean для совместимости
            // Спавним 5 раз для улучшения видимости
            for (int i = 0; i < 5; i++) {
