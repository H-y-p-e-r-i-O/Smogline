...
import com.smogline.menu.BlastFurnaceMenu;
    ...
    private void renderSlotDirectionTooltip(GuiGraphics guiGraphics, int mouseX, int mouseY) {
        ...
        for (int i = 0; i < slots.length; i++) {
            ...
            if (slot != null && !slot.hasItem() && isHovering(slot.x, slot.y, 16, 16, mouseX, mouseY)) {
                Component direction = Component.translatable("direction.smogline." + menu.getConfiguredDirectionForSlot(i).getName());
                Component tooltip = Component.translatable("gui.smogline.blast_furnace.accepts", direction)
