package com.smogline.particle.explosions.nuclear.small;

import com.smogline.particle.explosions.AbstractExplosionParticle;
import com.smogline.particle.explosions.basic.CameraShakeHandler;
...
public class DarkWaveSmokeParticle extends AbstractExplosionParticle {
    ...
    private static final float MAX_SHAKE_INTENSITY = 2.0F;

    // Расстояние, после которого эффект становится минимальным (в блоках)
    private static final double MAX_DISTANCE_FOR_EFFECT = 18.0;

    // Минимальная интенсивность (для далёких частиц)
    private static final float MIN_SHAKE_INTENSITY = 0.3F;
    ...
    public DarkWaveSmokeParticle(ClientLevel level, double x, double y, double z,
        ...
        this.quadSize = 1F + this.random.nextFloat() * 0.6F;
    ...
    public static class Provider extends AbstractExplosionParticle.Provider<DarkWaveSmokeParticle> {
        public Provider(SpriteSet sprites) {
            super(sprites, DarkWaveSmokeParticle::new);
        }
    }
}
