package com.smogline.radiation;

// Обработчик радиации для игроков

import com.smogline.block.custom.machines.armormod.util.ArmorModificationHelper;
...
import com.smogline.network.RadiationDataPacket;
import com.smogline.damagesource.ModDamageSources;
import com.smogline.hazard.HazardSystem;
import com.smogline.hazard.HazardType;
...
public class PlayerHandler {
    ...
    private static final String NBT_KEY_PLAYER_RADIATION = "smogline_player_radiation";
                    ...
                    .then(Commands.argument("targets", EntityArgument.entities())
                        .then(Commands.literal("clear")
                            .executes(ctx -> {
                                ...
                                ctx.getSource().sendSuccess(() -> Component.translatable("commands.smogline.rad.cleared", count[0]), true);
                                return count[0];
                            })
                        )
                        .then(Commands.literal("add")
                            .then(Commands.argument("amount", FloatArgumentType.floatArg())
                                .executes(ctx -> {
                                    ...
                                    ctx.getSource().sendSuccess(() -> Component.translatable("commands.smogline.rad.added", amount, count[0]), true);
                                    return count[0];
                                })
                            )
                        )
                        .then(Commands.literal("remove")
                            .then(Commands.argument("amount", FloatArgumentType.floatArg())
                                .executes(ctx -> {
                                    ...
                                    ctx.getSource().sendSuccess(() -> Component.translatable("commands.smogline.rad.removed", amount, count[0]), true);
                                    return count[0];
                                })
                            )
                        )
                    ...
                    .then(Commands.literal("clear")
                        .executes(ctx -> {
                            ...
                            if (self instanceof Player p) {
                                setPlayerRads(p, 0F);
                                ctx.getSource().sendSuccess(() -> Component.translatable("commands.smogline.rad.cleared.self"), true);
                                return 1;
                            }
                            return 0;
                        })
                    ...
                    .then(Commands.literal("add")
                        .then(Commands.argument("amount", FloatArgumentType.floatArg())
                            .executes(ctx -> {
                                ...
                                if (self instanceof Player p) {
                                    incrementPlayerRads(p, amount);
                                    ctx.getSource().sendSuccess(() -> Component.translatable("commands.smogline.rad.added.self", amount), true);
                                    return 1;
                                }
                                return 0;
                            })
                    ...
                    .then(Commands.literal("remove")
                        .then(Commands.argument("amount", FloatArgumentType.floatArg())
                            .executes(ctx -> {
                                ...
                                if (self instanceof Player p) {
                                    decrementPlayerRads(p, amount);
                                    ctx.getSource().sendSuccess(() -> Component.translatable("commands.smogline.rad.removed.self", amount), true);
                                    return 1;
                                }
                                return 0;
                            })
    ...
    private static int executeClientRenderToggle(com.mojang.brigadier.context.CommandContext<net.minecraft.commands.CommandSourceStack> context) {
        try {
            var current = com.smogline.client.render.shader.RenderPathManager.getCurrentPath();
            boolean shaderActive = com.smogline.client.render.shader.ShaderCompatibilityDetector.isExternalShaderActive();
            
            // Проверяем попытку включить VBO при активном шейдере
            if (current == com.smogline.client.render.shader.RenderPathManager.RenderPath.IMMEDIATE_FALLBACK && shaderActive) {
            ...
            var newPath = current == com.smogline.client.render.shader.RenderPathManager.RenderPath.VBO_OPTIMIZED
                ? com.smogline.client.render.shader.RenderPathManager.RenderPath.IMMEDIATE_FALLBACK
                : com.smogline.client.render.shader.RenderPathManager.RenderPath.VBO_OPTIMIZED;
            
            com.smogline.client.render.shader.RenderPathManager.forceSetPath(newPath);
